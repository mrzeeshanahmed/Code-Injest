    ...
    analyze(files) {
        ...
        for (const file of files) {
            ...
            if (!languageBreakdown[lang])
                languageBreakdown[lang] = { files: 0, lines: 0, percentage: 0 };
            languageBreakdown[lang].files++;
            // Large file detection
            if (file.size > 100000)
                largeFiles.push({ path: file.path, size: file.size });
        ...
        for (const hash in hashToFiles) {
            if (hashToFiles[hash].length > 1)
                duplicatedFiles.push(...hashToFiles[hash]);
        }
        return {
            statistics: {
