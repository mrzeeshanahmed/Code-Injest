...
export class BinaryDetector {
    private static binaryExtensions = [
    ...
    isBinary(buffer: Buffer, filePath?: string): boolean {
        ...
        if (buffer.length > 2) {
            // UTF-8 BOM
            if (buffer[0] === 0xEF && buffer[1] === 0xBB && buffer[2] === 0xBF) return false;
            // UTF-16 LE BOM
        ...
        if (controlRatio > 0.3) return true;
        ...
        try {
            ...
            if (replacementCount / sample.length > 0.01) return true;
        } catch {
            return true;
        }
